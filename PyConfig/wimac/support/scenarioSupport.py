...
from wimac.support.WiMACParameters import ParametersSystem
...
def setupHexagonalScenario(config, ofdmaPhyStations):
    ...
    for pos in listOfAccessPointPositions:
        ofdmaPhyStations[n].mobility.mobility.setCoords(Position(pos[0],pos[1],config.parametersSystem.height['AP']))
...
def setupRelayScenario(config, ofdmaPhyStations, associations):
    ...
    n = 0
    for pos in listOfAccessPointPositions:
        ofdmaPhyStations[n].mobility.mobility.setCoords(Position(pos.x,pos.y,config.parametersSystem.height['AP']))
        #  place User Terminals of the AP
        u = 0
