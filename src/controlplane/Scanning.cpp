...
ScanningBS::messageNewFrame()
{
		...
		it != doMOB_SCN_REQ_.end();)
	{
		...
		cis = friends_.connectionManager->getAllCIForSS(utID);
		for ( service::ConnectionManager::ConnectionIdentifiers::iterator it2
...
void
ScanningSS::start(service::scanningStrategy::Interface::Stations stationsToScan,
...
ScanningSS::messageNewFrame()
{
	...
	if ( scan_.scan == true )
	{
		...
		{   // Start Scanning
			...
			for( service::ConnectionManager::ConnectionIdentifiers::iterator
...
void
ScanningSS::resultScanning(CIRMeasureInterface::MeasureValues measureValues)
{
	LOG_INFO( getFUN()->getName(),
