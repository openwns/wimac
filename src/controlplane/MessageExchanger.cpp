...
STATIC_FACTORY_REGISTER_WITH_CREATOR(wimac::controlplane::MessageExchanger,
...
MessageExchanger::start(MessageExchangerCallBackInterface* callBackInterface)
{
	...
	dll::Layer2::StationIDType peerID = getPeerID(0);
...
MessageExchanger::doOnData(const wns::ldk::CompoundPtr& compound)
{
	...
	dll::Layer2::StationIDType stationID =
		dynamic_cast<dll::Layer2*>(getFUN()->getLayer())->getID();

	dll::Layer2::StationIDType peerID = 0;
	...
	if ( command->peer.peerID != dynamic_cast<dll::Layer2*>
...
MessageExchanger::doWakeup()
{
		...
		it != compoundQueue_.end(); )
	{
		...
		/*********** Get peerID *********************/
		dll::Layer2::StationIDType peerID;
...
MessageExchanger::onFUNCreated()
{
	...
	friends_.connectionManager = dynamic_cast<dll::Layer2*>(getFUN()->getLayer())
...
MessageExchanger::createMessage(const StationID peerID, const MessageType messageType)
{
	...
	cis = friends_.connectionManager->getOutgoingConnections(peerID);
	assure(!cis.empty(),
		   "MessageExchanger::createMessage: Haven't get ConnectionIdentifiers for PeerID!\n");

	service::ConnectionManager::ConnectionIdentifiers::const_iterator
		it = cis.begin();
	for(; it != cis.end(); ++it)
	{
...
{
	ConnectionIdentifierPtr ci;
	dll::Layer2::StationIDType stationID = -1;
	dll::Layer2::StationIDType peerID = -1;
	...
	stationID = dynamic_cast<dll::Layer2*>(getFUN()->getLayer())->getID();
