...
SetupConnectionBS::doOnDSA_REQ(const wns::ldk::CompoundPtr& compound)
{
    ...
    int ulQoSPriorityREQ;
    int dlQoSPriorityREQ;
...
void
SetupConnectionSS::start(int qosPriority,
...
SetupConnectionSS::doOnDSA_RSP(const wns::ldk::CompoundPtr& compound)
{
    ...
    dsaCI = friends_.connectionManager->getConnectionWithID( cCommand->peer.id );
    assure( dsaCI,
            "SetupConnectionSS::doOnDSA_RSP: Didn't get ConnectionIdentifier "
            "from ConnectionManager for that compound CID!");
    ...
    ConnectionIdentifierPtr bCI (
        friends_.connectionManager
        ->getBasicConnectionFor(
            ConnectionIdentifier::StationID(
                dynamic_cast<Component*>(getFUN()->getLayer())
                ->getID() ) ) );
    assure(bCI->valid_,
           "SetupConnectionSS::doOnDSA_RSP: "
           "Didn't get ConnectionIdentifier from ConnectionManager!");
    friends_.connectionClassifier->activateCommand(
        newCompound->getCommandPool() );
    dynamic_cast<wns::ldk::ClassifierCommand*>
        ( friends_.connectionClassifier->getCommand(
            newCompound->getCommandPool() ) )
        ->peer.id = bCI->cid_;
