	...
	case Sending:
		{
			...
			phyCommand->local.pAFunc_->transmissionStop_ = now + getCurrentDuration() - Utilities::getComputationalAccuracyFactor();
			phyCommand->local.pAFunc_->phyMode_ = phyMode;

			setTimeout( getCurrentDuration() );
			LOG_INFO("Sending DL MAP in ", getFUN()->getName(), " and setting timeout to ", getCurrentDuration());
...
void
DLMapCollector::calculateSizes( const wns::ldk::CommandPool* commandPool, Bit& commandPoolSize, Bit& dataSize ) const
{
...
simTimeType DLMapCollector::getCurrentDuration() const
{
	double dataRate =
		phyMode->getDataRate();
