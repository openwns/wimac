...
void ContentionCollector::doSendData( const wns::ldk::CompoundPtr& compound )
{
    ...
    std::string stationType = StationType::toString( layer_->getStationType() );
    if( stationType == "AP" )
    {
        destinationID = connection->subscriberStation_;
    }
    else if( stationType == "UT"||  stationType == "RT" )
    {
    ...
    {
        /**
         * @todo In relay stations the direction of a compound can not be
    ...
    func->destination_ =
        TheStationManager::getInstance()->getStationByID( destinationID )
        ->getNode();

    wns::simulator::Time duration = getDuration( compound );
