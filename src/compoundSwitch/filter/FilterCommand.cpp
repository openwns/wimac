...
{
    friends_.commandProvider = compoundSwitch_->
        findFUNFriend(friends_.commandProviderName);
}
...
{
    if( compoundSwitch_->getCommandProxy()->
        commandIsActivated( compound->getCommandPool(), friends_.commandProvider ) )
        return true;
