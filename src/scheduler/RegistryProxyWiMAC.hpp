...
namespace wimac { namespace scheduler {
	...
	public:
		...
		wns::CandI estimateTxSINRAt(const wns::scheduler::UserID user);
		wns::CandI estimateRxSINROf(const wns::scheduler::UserID user);
		...
		virtual wns::scheduler::ConnectionSet filterReachable(wns::scheduler::ConnectionSet connections); // added to RegistryProxyInterface
