...
//	wimac::scheduler::PseudoBWRequestGenerator,
...
void
PseudoBWRequestGenerator::setScheduler(wimac::scheduler::SchedulerInterface* scheduler)
{
...
struct UpgoingConnection
	: public std::unary_function< wimac::service::ConnectionManager::ConnectionIdentifierPtr, bool>
{
	explicit UpgoingConnection(int myID): myID_( myID) {}
	bool operator()(const wimac::service::ConnectionManager::ConnectionIdentifierPtr& ptr)
	{
...
PseudoBWRequestGenerator::wakeup() {
	...
	service::ConnectionManager::ConnectionIdentifiers allBasicConnIDs =
	...
	for (ConnectionIdentifier::List::const_iterator iter = allBasicConnIDs.begin();
		...
		Component* peerComponent = dynamic_cast<wimac::Component*>( component_->getStationManager()->getStationByID(peerStationId) );

		assure(peerComponent, "Invalid peer layer pointer");
		assure(peerComponent->getNode(), "No valid Node pointer in peer FUN");

		//FIXME BWrequest shortcut
		wimac::service::ConnectionManager::ConnectionIdentifiers cis = component_->getManagementService<service::ConnectionManager>
