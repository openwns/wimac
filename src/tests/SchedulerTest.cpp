...
const uint32
SchedulerTest::burstSize = 2;
const uint32
SchedulerTest::noOfBursts = 3;
const double
SchedulerTest::PDUtransmitDuration = 0.01;
...
void SchedulerTest::setUp()
{
    ...
    ss<<"from wns.Buffer import Dropping\n"
    ...
    layer->getProxy()->addFunctionalUnit( mybuffers.get() );
    layer->getProxy()->addFunctionalUnit( myClassifier.get() );
    layer->getProxy()->addFunctionalUnit( myScheduler.get() );
...
void SchedulerTest::testTransmitDuration()
{
    ...
    CPPUNIT_ASSERT( fabs(PDUtransmitDuration-(sample[1] - sample[0])) < (0.001 * PDUtransmitDuration));
    ...
    CPPUNIT_ASSERT( fabs(frameDuration-(sample[2] - sample[0])) < (0.001 * frameDuration));
}
