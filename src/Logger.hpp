...
namespace wimac {
	...
	public:
		...
		Logger( int32_t priority, const std::type_info& type ):
		...
		Logger* prepareForMessage(int32_t priority, const std::type_info& type)
		{
			priority_ = priority;
		...
		/** @todo These methods should also work if the first parameter is not
		 * a char* or a std::string.
		 */
		template<class T1>
		void send( T1 t1 )
		{
