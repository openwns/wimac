...
FUReseter::FUReseter(wns::ldk::ManagementServiceRegistry* msr, const wns::pyconfig::View& config ) :
	...
	fun_( dynamic_cast<dll::Layer2*>( getMSR()->getLayer() )
		  ->getFUN() )
{
...
void
FUReseter::resetFlowSeparator(ConnectionIdentifier::Ptr ci)
{
	...
	uint32_t oldQoSCategory = 0;
	assure(false, "fix above code if this is used");

	/// \warning Its a workaround to probe only the outgoing buffer size
	dll::Layer2::StationType stationType;
	stationType = static_cast<dll::Layer2*>(getMSR()->getLayer())
		->getStationType();
...
void
FUReseter::resetCID(ConnectionIdentifier::Ptr ci)
{
