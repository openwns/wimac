...
DeadStationDetect::DeadStationDetect( wns::ldk::ManagementServiceRegistry* msr,
    ...
    timeToLive_(config.get<wns::simulator::Time>("timeToLive")),
    friends_(),
...
DeadStationDetect::notifyActivityFor(CID cid)
{
    ...
    stationID = dynamic_cast<Component*>(getMSR()->getLayer())->getID();
...
DeadStationDetect::onMSRCreated()
{
    ...
    friends_.newFrameProvider = dynamic_cast<Component*>( getMSR()->getLayer() )
...
DeadStationDetect::adjustStations()
{
    ...
    for( ConnectionIdentifiers::const_iterator cisIT = cis.begin();
         cisIT != cis.end(); cisIT++)
    {
        ...
        it != stationLastActive_.end(); )
    {
        ...
        ConnectionIdentifiers::const_iterator cisIT = cis.begin();
...
void
DeadStationDetect::deleteDeadStations(const wns::simulator::Time deltaSimTime)
{
    wns::simulator::Time currentSimTime = wns::simulator::getEventScheduler()->getTime();
        ...
        it != stationLastActive_.end(); )
    {
        ...
        MESSAGE_SINGLE(NORMAL, logger_,
                       dynamic_cast<Component*>(getMSR()->getLayer())->getName()
                       << ": Station:" << cpIt->first
...
DSDSensor::onFUNCreated()
{
    ...
    friends_.deadStationDetect = dynamic_cast<Component*>(getFUN()->getLayer())
