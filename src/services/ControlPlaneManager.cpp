...
ControlPlaneManagerSS::ControlPlaneManagerSS( wns::ldk::ManagementServiceRegistry* msr,
	...
	associating_(new service::Associating(
	   static_cast<dll::Layer2*>( getMSR()->getLayer() ),
	   this, config) ),
...
void
ControlPlaneManagerSS::start(StationID /*associateTo */, QoSCategory qosCategory)
{
...
{
	friends_.connectionManager = dynamic_cast<dll::Layer2*>( getMSR()->getLayer() )
		->getManagementService<service::ConnectionManager>
		(friends_.connectionManagerName);

	friends_.phyUser = dynamic_cast<dll::Layer2*>( getMSR()->getLayer() )
		->getFUN()->findFriend<PhyUser*>(friends_.phyUserName);

	//associating_->onFUNCreated();
	//dissociating_->onFUNCreated();
}
...
ControlPlaneManagerSS::stateScanning(const MeasureValues& measureValues)
{
	...
	ownID = dynamic_cast<dll::Layer2*>(getMSR()->getLayer())->getID();
