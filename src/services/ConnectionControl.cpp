	...
	associatedWithID_(0)
{
	if ( !config.isNone("associatedWith") )	{
		associatedWithID_ = config.get<dll::Layer2::StationIDType>("associatedWith");
	}
}
...
void
ConnectionControl::associateTo(dll::Layer2::StationIDType associateTo,
							   ConnectionIdentifier::QoSCategory qosCategory)
{
	...
	dll::Layer2::StationIDType stationID = layer->getID();

	wimac::Component* destination ( static_cast<wimac::Component*>
								 ( layer->getStationManager()->getStationByID( associateTo ) ) );
	...
	dll::Layer2* layer2 =
		dynamic_cast<dll::Layer2*>(getCSR()->getLayer());
											 ...
											 ConnectionIdentifier::QoSCategory qosCategory)
{
	...
	wimac::Component* associatedWith = static_cast<wimac::Component*>
		( dll::TheStationManager::getInstance()->getStationByID(associatedWithID) );
