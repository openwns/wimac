...
ControlPlaneManagerSimpleSS::ControlPlaneManagerSimpleSS(wns::ldk::ManagementServiceRegistry* msr,
	...
	: ManagementService(msr),
	  ...
	  component_(static_cast<wimac::Component*>( getMSR()->getLayer() )),
...
void
ControlPlaneManagerSimpleSS::start(StationID associateTo, QoSCategory qosCategory)
{
	// Probe settings
	// Set access point to probe IDProvider, we're associated/associating to.

	wns::probe::bus::ContextProviderCollection& cpc =
		dynamic_cast<dll::Layer2*>( getMSR()->getLayer() )
...
void
ControlPlaneManagerSimpleSS::associateAndConnectTo(StationID associateTo, QoSCategory qosCategory)
{
  assure( associateTo,
  ...
  wimac::Component* destination = static_cast<wimac::Component*>
	  ( component_->getStationManager()->getStationByID( associateTo ) );
													   ...
													   QoSCategory qosCategory)
{
	...
	wimac::Component* associatedWith = static_cast<wimac::Component*>
		( component_->getStationManager()->getStationByID(associatedWithID) );
...
// 	wimac::Component* associatedWith = static_cast<wimac::Component*>
// 		( component_->getStationManager()->getStationByID(friends_.connectionManager->getConnectionWithID(0)->baseStation_) );
// 	assureType( associatedWith, wimac::Component* );

// 	LOG_INFO(component_->getName(),
// 			 ": associatedWith: ", associatedWith->getID());
...
// 	else
// 		return associatedWith
// 			->getManagementService<ControlPlaneManagerSimpleSS>(msName_)
